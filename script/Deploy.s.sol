...
/**
 * @title DeployTickHarberger
 * @notice Deploys TickHarberger deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployTickHarberger is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(TickHarbergerHook).creationCode;
        ...
        console2.log("TickHarberger ", predicted);
        ...
        vm.startBroadcast();
        TickHarbergerHook hook = new TickHarbergerHook{salt: salt}(manager);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("TickHarberger")),
