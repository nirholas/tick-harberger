...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(TickHarbergerHook).creationCode, abi.encode(IPoolManager(address(manager)))
        );
        hook = new TickHarbergerHook{salt: salt}(IPoolManager(address(manager)));
    ...
    function _openPool() private {
        ...
        hook.configure(key, TickHarbergerHook.Config({rangeWidth: 600, rentRateBps: 500, maxFeePips: 50_000, minDeposit: 1e15}));
