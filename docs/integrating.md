# Integrating TickHarberger

## From Solidity

```solidity
import {TickHarbergerHook} from "tick-harberger/src/hooks/TickHarbergerHook.sol";
...
hook.configure(key, TickHarbergerHook.Config({ /* see the README */ }));
...
const address = hookAddress("tick-harberger", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "TickHarberger"
...
More at https://tick-harberger.pages.dev.
